...
window.reloadvscode=function(){
    window.vscode.postMessage({
        action:'reload',
    });
}

window.vscodeChangeTitle=function(title){
    window.vscode.postMessage({
        action:'changeTitle',
        title:title,
    });
}
window.vscodeAlert=function(message){
    window.vscode.postMessage({
        action:'alert',
        message:message,
    });
...
function commando() {
  ...
  this.search = "javascript loop array";//getParameterByName("q");
  ...
  this.vscode_language="whatever";
  this.addingAnswerFromText=false;
}
    ...
    document.addEventListener('keyup', function(event) {
        ...
        if(event.key=="a"){
            this.injectScript('codemirror/lib/codemirror.js', 'body',function(){
                //here we need to be done injectting script
...
commando.prototype.getAnswers=function(){
  return new Promise(function (resolve, reject) {
    ...
    var grepper_user_id = localStorage.getItem('grepper_user_id');
    var grepper_access_token  = localStorage.getItem('grepper_access_token'); 


    makeRequest('GET', this.endpoint+"/vscode_get_answers.php?v=2&s="+encodeURIComponent(this.search)+"&u="+this.user_id+this.tifgrpr+"&HTTP_X_AUTH_TOKEN="+grepper_access_token+"&HTTP_X_AUTH_ID="+grepper_user_id,{},true).then(function(data){
        var results=JSON.parse(data);

        if(results.reason =="Unauthorized"){
            this.showAddonRequiredMessageAndClose();
            return;
        }
...
commando.prototype.displayAnswerBox =function(editingAnswer) {
    ...
    if(document.getElementById("grepper_add_answer_button")){
        document.getElementById("grepper_add_answer_button").style.display="none";
    }
    ...
    if(this.addingAnswerFromText){
        this.codeResults.textContent=this.addingAnswerFromText;    
    }
    ...
    if(!this.search){
        window.vscodeAlert("Please enter search term for this code answer");
        this.newAnswerTitle.style.border="2px solid red";
        return;
    }
...
//here we setup the answer title if it is an option
    if(this.addingAnswerFromText){
        this.newAnswerTitle = document.createElement("input");
        //this.newAnswerTitle.value="ok";
        this.newAnswerTitle.setAttribute("id","grepper_new_answer_title");
        this.newAnswerTitle.setAttribute("placeholder","Enter a Search Term");

        var that=this;
        this.newAnswerTitle.addEventListener("change", function(el){
            that.search=this.value;
            window.vscodeChangeTitle(this.value);
        });


        this.statsDom.insertBefore(this.newAnswerTitle,this.statsDom.childNodes[0]);
    }
}
...
commando.prototype.deleteAnswer = function(id) {
      //var r = window.vscodeconfirm("Are you sure you want to delete this answer?");
      //if (r == true) {
                makeRequest('POST', this.endpoint+"/delete.php?id="+id+"&u="+this.user_id).then(function(data){
                window.reloadvscode();
            }.bind(this));
...
commando.prototype.showNoAnswersMessage =function() {
      document.getElementById("no_answers_message").textContent="Hmm, nothing obvious came up for the search “"+this.search+"”. Try expanding the search results by pressing “o” to get more of other people's answers. Or if you know you have put in an answer press “m” to get more of your own answers.";
      document.getElementById("no_answers_message").style.display="block";
}
...
//  }
        ...
        document.getElementById("grepper_title_1").appendChild(addAnswerButton);
...
commando.prototype.languageToSelect =function(l){
    ...
    var options=this.languageSelectOptions;
...
commando.prototype.injectScript = function(file, node ,callback) {
  if(this.loadedCodeMirrorModes.indexOf(file) === -1){
    ...
    var script = document.createElement('script');
        script.src = this.mediaPath+"/"+file;
  ...
  return new Promise(function (resolve, reject) {
   ...
   var postData={
    ...
    makeRequest('POST', this.endpoint+"/get_more_answers.php?others="+others+this.tifgrpr,JSON.stringify(postData)).then(function(r){
      ...
      if(results.length){
            document.getElementById("no_answers_message").style.display="none";
      }
...
//first we look at search term
//then we look at the file type they came from
//note: the langauge has to exist in there code languages
//maybe we should auto add if its missing?
commando.prototype.guessCodeLanguage=function(callback){
    ...
    //if vscode lang key present and in the options lets assume that language
    for (var key in options) {
        if(this.vscode_language === key){
            callback(key); return;
        }
    }
...
commando.prototype.setupAddingNewAnswer=function(message){
if(this.getStatsDom()){
    this.addingAnswerFromText=message.answer_text;
    this.injectScript('codemirror/lib/codemirror.js', 'body',function(){
       this.displayAnswerBox();
    }.bind(this));
    ...
    makeRequest('GET',this.endpoint+"/check_auth.php?vscode=1").then(function(d){
...
commando.prototype.showAddonRequiredMessageAndClose=function(){
        window.vscodeAlert("You need to Login to your Grepper account to use this feature. Login by clicking the Grepper Icon on the activity bar to the left.");
        window.vscode.postMessage({
            action:'closeWindow'
        });
...
window.addEventListener('message', function(event) {
    ...
    if(message.command=="init"){
        localStorage.setItem("grepper_user_id",message.grepper_user_id);
        localStorage.setItem("grepper_access_token",message.grepper_access_token);
    
        //setup the user code languages
        var items=message.grepper_user_langs;
        var myOptions={};
        for (var key in items) {
            if(items[key].enabled){
                myOptions[items[key].lkey]=items[key].name;
            }
        }

        var co=new commando();
            co.user_id = message.grepper_user_id;
            co.search = message.search_term;
            co.languageSelectOptions=myOptions;
            co.mediaPath=message.mediaPath;
            if(message.language){
                co.vscode_language=message.language;
            }
            if(message.answer_text){
                document.getElementById("grepper_title_1").textContent="Add a Grepper Answer";
                co.setupAddingNewAnswer(message);
            }else{
                co.getAnswers();
                co.settupKeyBindings();
            }
    }
});
